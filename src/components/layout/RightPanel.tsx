...
const toneClass: Record<Tone, string> = {
  ...
  neutral: "border-border-subtle bg-card text-muted-foreground",
};
...
export function RightPanel({ activeTab, onTabChange, workspaceState, isMobile, onClose }: RightPanelProps) {
  ...
  if (!isMobile && collapsed) {
    return (
      <div className="w-8 border-l border-border-subtle bg-panel flex flex-col items-center py-2 gap-1.5 shrink-0">
        {tabs.map((tab) => (
