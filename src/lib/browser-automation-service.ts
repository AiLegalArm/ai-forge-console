    ...
    try {
      ...
      return {
        session: {
          ...
          runState: runtimeState === "completed" ? "completed" : "failed",
          runtimeState,
          sessionState: toSessionState(runtimeState),
          resultState,
          currentStepId,
          scenario,
          consoleSummary: consoleEvents.map((e) => `${e.level}: ${e.message}`),
          networkSummary: networkEvents.map((e) => `${e.method} ${e.url} → ${e.statusCode}`),
          consoleEvents,
          networkEvents,
