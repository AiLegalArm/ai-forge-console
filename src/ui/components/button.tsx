...
const variants: Record<ButtonVariant, string> = {
  primary: "bg-primary/90 text-primary-foreground hover:bg-primary active:bg-primary/80",
...
export function Button({ variant = "ghost", loading, selected, disabled, className, children, ...props }: UIButtonProps) {
  return (
    <button
      ...
      className={cn(
        "h-8 px-2.5 text-[11px] font-medium rounded-sm transition-colors inline-flex items-center gap-1.5 shrink-0",
        "disabled:opacity-40 disabled:pointer-events-none",
        variants[variant],
        selected && "bg-primary/12 text-primary border border-primary/30",
        loading && "opacity-60",
        className,
      )}
      {...props}
